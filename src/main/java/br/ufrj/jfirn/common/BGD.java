...
public class BGD {
	...
	public static double cdf(double a, double b, double rho) {
		if((a <= 0) && (b <= 0) && (rho <= 0)) {
			final double aprime = a/FastMath.sqrt(2 * (1 - rho*rho));
			final double bprime = b/FastMath.sqrt(2 * (1 - rho*rho));
			double sum = 0;
			...
			final double denum = FastMath.sqrt(a * a - 2 * rho * a * b + b * b);
			final double rho1 = ((rho * a - b) * FastMath.signum(a)) / denum;
			final double rho2 = ((rho * b - a) * FastMath.signum(b)) / denum;
			final double delta = (1 - FastMath.signum(a) * FastMath.signum(b)) / 4d;
